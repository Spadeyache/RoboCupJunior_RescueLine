#!/usr/bin/env python3
"""
Convert YOLOv8 ONNX model to K230D .kmodel format using nncase 2.9.0
Applies INT8 quantization for KPU v3.0 deployment.

CRITICAL NOTES:
  - K230D / ai2d: float32 calibration in [0, 255] with **no** /255 so the kmodel matches the
    chip’s hardware preprocessing (ai2d → KPU). This is required to use ai2d acceleration correctly.
  - nncase 2.9 PTQ: feed float32 aligned with ONNX f32 input (not uint8 tensors).
  - Calibration color space: RGB (OpenCV is BGR → convert). Layout: NCHW.

Usage:
  python convert_to_kmodel_uint8.py model.onnx \
      --calib-data /path/to/images \
...
def verify_onnx_model(onnx_path: str, img_height: int, img_width: int) -> dict:
    ...
    for out in model.graph.output:
        ...
        if len(shape) == 3:
            ...
            num_classes = rows - 4
            print(f"[ONNX]   ✓ YOLOv8 output: {anchors} anchors, "
...
def generate_calibration_data(
    ...
    """
    Load images as float32 [0, 255] in NCHW — NO /255.
    Matches K230D ai2d + KPU path and nncase PTQ (ONNX f32 input).
    """
    import cv2
    from glob import glob

    print(f"\n{'='*60}")
    print(f"[CALIB] Generating calibration data (float32 [0,255], NO /255)")
    print(f"{'='*60}")
    ...
    for idx, img_path in enumerate(image_files):
        try:
            ...
            # 3. float32 [0, 255] — NO /255 (K230D ai2d path + nncase PTQ expects f32, not u8)
    ...
    # Sanity check — if max is still ~1.0, normalization was accidentally applied
    if all_data.max() < 2:
...
def convert_to_kmodel(
    ...
    print(f"\n{'='*60}")
    print(f"  YOLOv8 → K230D kmodel Converter  (nncase 2.9.0)")
    ...
    try:
        ...
        print(f"[COMPILE]   sample min/max     : {ptq_data[0][0].min():.0f} / {ptq_data[0][0].max():.0f}  ← must be [0, 255]")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert YOLOv8 ONNX → K230D kmodel (nncase 2.9.0) — f32 [0,255] calibration",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("onnx", type=str,
                        help="Path to YOLOv8 .onnx model")
    parser.add_argument("--output", type=str, default="./output",
                        help="Output directory for .kmodel")
    parser.add_argument("--calib-data", type=str, default=None,
                        help="Path to calibration images (STRONGLY recommended)")
    parser.add_argument("--img-height", type=int, default=480,
                        help="Model input height (must match ONNX export size)")
    parser.add_argument("--img-width", type=int, default=640,
                        help="Model input width (must match ONNX export size)")
    parser.add_argument("--target", type=str, default="k230",
                        help="nncase target device")
    parser.add_argument("--num-samples", type=int, default=100,
                        help="Number of calibration images to use")
    parser.add_argument("--quant-type", type=str, default="uint8",
                        choices=["uint8", "int8"],
                        help="Activation quantization type")
    parser.add_argument("--w-quant-type", type=str, default="uint8",
                        choices=["uint8", "int8"],
                        help="Weight quantization type")
    parser.add_argument("--calib-method", type=str, default="Kld",
                        choices=["Kld", "NoClip", "AbsMax"],
                        help="PTQ calibration method")

    args = parser.parse_args()

    if not args.calib_data:
        print("\n" + "!"*60)
        print("  WARNING: No --calib-data provided!")
        print("  Random calibration will produce a low-quality model.")
        print("!"*60)
        resp = input("\n  Continue with random calibration? [y/N]: ").strip().lower()
        if resp != "y":
            print("Aborted.")
            sys.exit(0)
