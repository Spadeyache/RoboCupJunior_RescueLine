...
bool Camera_Init() {
    ...
    if (s) {
        ...
        s->set_aec_value(s, 60);      // 200  Manual exposure: 0 to 1200 (Higher = brighter/slower shutter)
        s->set_aec2(s, 0);             // Disable night-mode AEC when in manual

        // --- Manual gain ---
        s->set_gain_ctrl(s, 0);        // Disable AGC (0 = Manual)
        s->set_agc_gain(s, 1); //10       // Manual gain: 0 to 30 (Higher = brighter but more noise)
